...
class Display:
    ...
    def print_packet(self, info_dict):
        ...
        src_host  = info_dict.get('src_host',  '')
        dst_host  = info_dict.get('dst_host',  '')
        ...
        # Build IP display strings (append hostname in parentheses if resolved)
        src_label = f"{src_ip} ({src_host})" if src_host else src_ip
        dst_label = f"{dst_ip} ({dst_host})" if dst_host else dst_ip

        # Print IP addresses with optional ports
        if src_port and dst_port:
            print(f"  {Fore.LIGHTBLUE_EX}IP: {src_label}:{src_port} → {dst_label}:{dst_port}{Style.RESET_ALL}")
        else:
            print(f"  {Fore.LIGHTBLUE_EX}IP: {src_label} → {dst_label}{Style.RESET_ALL}")
