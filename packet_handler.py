...
from utils import get_protocol_name, get_timestamp, format_bytes, resolve_hostname

class PacketHandler:
    """Core class for processing and parsing captured packets"""

    def __init__(self, display, resolve=False):
    ...
    def process(self, packet):
        ...
        # Reverse DNS resolution (only when -r flag is set)
        if self.resolve and info['src_ip'] != 'N/A':
            info['src_host'] = resolve_hostname(info['src_ip']) or ''
            info['dst_host'] = resolve_hostname(info['dst_ip']) or ''
